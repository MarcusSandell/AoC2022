...
path = "Day16//Day16.txt"

valves =  {} # Flow rate of each valve
tunnels = {} # Connections to each valve
...
for valve in valves:
    if valve != "AA" and not valves[valve]: # Skips empty valves
        continue
    ...
    while queue: # Creates map to every non empty node
...
def dfs(time, valve, bitmask): # Bitmask stores currently open valves
    ...
    for neighbor in dists[valve]:
        ...
        if bitmask & bit: # Checks if valve is already open
            continue
